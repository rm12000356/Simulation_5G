...
y = np.zeros(len(pilot_p), dtype=complex)  # Received vector for pilots
ind_temp = 0 # index for pilots
for k in range(len(data_with_pilots)):
    Rx[k] = true_H * data_with_pilots[k] + noise #filling the received data that will be affected by the channel H and noise
    if(k == pilot_p[ind_temp]):
        y[ind_temp] = true_H * pilot_symbol + noise # filling pilots after being affected by channel H and noise
        ind_temp += 1
...
plot_constellation(mod_data, log2(M),title=str(M)+"-QAM Constellation")
